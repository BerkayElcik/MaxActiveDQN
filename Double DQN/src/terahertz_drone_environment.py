...
class thz_drone_env(gym.Env):
    ...
    def __init__(self, render_mode=None, n_channels=3000, P_T=1, freq_of_movement=0.1, no_of_actions=1000):
    ...
    def calc_capacity(self, channel_obs, loss, noise):
        ...
        for channel_iter, power_iter in enumerate(power_alloc):
            ...
            SNR_curve_value=1/(path_loss*noise_power)



            SNR_list.append(SNR)
            SNR_curve.append(SNR_curve_value) # this is used only to see what SNR values would be if all the channels are selected. We use this to select the channels that are above the threshold SNR for MaxActive.
        ...
        max_snr=max(SNR_curve)
        min_snr=min(SNR_curve)
        ...
        with np.printoptions(threshold=np.inf):
            print(SNR_list)
        print("no of nonzero")
        print(np.count_nonzero(SNR_list))
        """


        return Capacity, max_snr, min_snr, SNR_list, SNR_curve
    """
    ...
    def dbm2mW(self, value):
        result = 10 ** (value / 10)
        return result

    def mW2dbm(self, value):
        result= 10*math.log10(value)
        return result


    def action2threshold(self, action, max_threshold, min_threshold):
        if max_threshold==None:
            max_threshold=self._max_snr
        if min_threshold==None:
            min_threshold=self._min_snr



        #predetermined_starting_value_for_s_db=30
        #predetermined_starting_value_for_s=self.dbm2mW(predetermined_starting_value_for_s_db)
        #start=predetermined_starting_value_for_s
        #delta_s=start/(self.no_of_actions-1)
        #max_threshold=1e-3
        #min_threshold=1e-7
        delta_s=(max_threshold-min_threshold)/(self.no_of_actions-1)
        threshold=action*delta_s+min_threshold
        #threshold_db= self.mW2dbm(threshold)

        return threshold

    def threshold2channels(self, threshold, snr_curve):
        """
        if channel_obs is None:
            channel_obs = self._channels
        ...
        #channel_obs=np.array(channel_obs)

        #channel_snr=np.zeros_like(channel_obs, dtype=np.float32)




        #snr_array = np.array(snr_list)
        snr_array=np.array(snr_curve)

        """
        print("condition")
        print(np.any(channel_obs == 1))
        
        if np.any(channel_obs == 1):
        ...
        """
        print("no of selected channels")
        print(np.sum(selected_channels))
        """

        """
        print("total snr")
        print(np.sum(channel_snr))
    ...
    def step(self, action):
        ...
        self._max_snr= observation["max_snr"]
        ...
        Capacity, self._max_snr, self._min_snr, self._snr_list, self._snr_curve= self.calc_capacity(self._channels, self._loss, self._noise) # new capacity
